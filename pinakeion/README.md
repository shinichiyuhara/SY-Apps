...
- Indexes ZIP / CBZ / RAR / CBR / 7z / TAR (gz / bz2 / xz / Zstandard) / PDF plus every common image format (JPEG, PNG, GIF, BMP, TIFF, WebP, AVIF, HEIC/HEIF, JPEG 2000, PSD, ICO, APNG, TGA, OpenEXR).
...
- Three-pane macOS-native layout (sidebar / folder tree / grid or list), each pane independently collapsible (⌘⌃S sidebar, ⌘⌃D folder browser, ⌘I inspector), plus a resizable inspector with cover preview, parsed metadata, an interactive 5-star rating picker, and system-wide usage read from Spotlight (times opened by any app, last used, download source) shown separately from Pinakeion's own open count.
...
- Built-in Quick Preview (Space / ⌘Y) renders the first 5×4 pages of ZIP / CBZ / PDF so you can scan the artwork without launching an external reader; pages are disk-cached for instant re-open.
